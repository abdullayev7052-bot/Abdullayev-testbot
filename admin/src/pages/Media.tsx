          ...
          <div key={s.id} className={`card p-4 flex gap-4 ${!s.active ? "opacity-60" : ""}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {s.slides.map((sl) => <img key={sl.id} src={sl.image} title={`${sl.duration}s`} className="w-9 h-12 rounded-md object-cover border border-slate-200" />)}
              </div>
        ...
        {slideFor && (
          <div className="space-y-4">
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
              {slideFor.slides.map((sl) => (
                <div key={sl.id} className="relative group">
                  <img src={sl.image} className="w-full aspect-[9/16] object-cover rounded-lg" />
            ...
            <div className="border-t border-slate-100 pt-4 space-y-3">
              ...
              <ImageUpload value={slide.image || ""} onChange={(v) => setSlide({ ...slide, image: v })} hint="Vertikal (9:16) rasm tavsiya etiladi" />
              <div className="grid sm:grid-cols-3 gap-3">
                ...
                <div><label className="label">Havola (ixtiyoriy)</label><input className="input" placeholder="https://..." value={slide.link || ""} onChange={(e) => setSlide({ ...slide, link: e.target.value })} /></div>
...
export function BannersPage() {
  ...
  return (
    <div>
      <PageTitle title="Bannerlar" description="Bosh sahifadagi aylanma bannerlar. Havola: https://..., category:<Bito kategoriya ID> yoki product:<mahsulot ID>" actions={<button className="btn btn-primary" onClick={() => setEdit({ active: true, textColor: "#ffffff" })}><Plus size={16} /> Yangi banner</button>} />
      <div className="grid md:grid-cols-2 gap-3">
        {list.map((b, i) => (
          <div key={b.id} className={`card overflow-hidden ${!b.active ? "opacity-60" : ""}`}>
            <div className="relative h-36"><img src={b.image} className="w-full h-full object-cover" />{(b.title || b.subtitle) && <div className="absolute inset-0 bg-gradient-to-r from-black/50 to-transparent p-4 flex flex-col justify-center" style={{ color: b.textColor }}><div className="font-bold text-lg">{b.title}</div><div className="text-sm opacity-90">{b.subtitle}</div></div>}</div>
      ...
      <Modal open={!!edit} onClose={() => setEdit(null)} title={edit?.id ? "Bannerni tahrirlash" : "Yangi banner"}>
        {edit && (
          <div className="space-y-4">
            <ImageUpload value={edit.image || ""} onChange={(v) => setEdit({ ...edit, image: v })} hint="Tavsiya: 1200×480 px" />
            ...
            <div><label className="label">Havola</label><input className="input" placeholder="https://... yoki category:ID yoki product:ID" value={edit.link || ""} onChange={(e) => setEdit({ ...edit, link: e.target.value })} /></div>
