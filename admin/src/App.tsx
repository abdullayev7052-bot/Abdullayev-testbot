...
function Shell({ onLogout, b, theme }: { onLogout: () => void; b?: Branding; theme: ReturnType<typeof useTheme> }) {
  ...
  const nav = (
    <nav className="p-3 space-y-1 text-sm">
      ...
      {theme.canToggle && <button onClick={theme.toggle} className={`${link} w-full mt-2`}>{theme.theme === "dark" ? <Sun size={17} /> : <Moon size={17} />} {theme.theme === "dark" ? "Yorug' rejim" : "Tungi rejim"}</button>}
      <button onClick={onLogout} className={`${link} w-full mt-2 text-red-600`}><LogOut size={17} /> Chiqish</button>
    </nav>
  );
  const brand = (
